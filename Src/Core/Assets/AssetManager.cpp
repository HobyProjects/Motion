...
namespace Motion::Core
{
    ...
    std::shared_ptr<IShader> AssetManager::CreateShader(const std::string& name, const std::filesystem::path& shaderFile)
    {
        ...
        case RenderingAPI::OpenGL:
        {
            ...
            std::string source = ShaderBuilder::ReadShaderFiles(shaderFile);
            if (source.empty())
                return nullptr;
    ...
    std::shared_ptr<IShader> AssetManager::CreateShader(UUID uuid, const std::string& name, const std::filesystem::path& shaderFile)
    {
        if (s_AssetRegistry.find(s_AssetNameUUIDMap[name]) != s_AssetRegistry.end())
        {
            MOTION_CORE_WARN("Shader {0} already exists!", name);
        ...
        case RenderingAPI::OpenGL:
        {
            ...
            std::shared_ptr<IShader> shaderAsset = std::make_shared<GL_Shader>(uuid, name, shaderSources, shaderFile);
            s_AssetRegistry[shaderAsset->GetMetaData().AssetUUID] = shaderAsset;
            MOTION_CORE_INFO("Shader created: {0} from file {1}", name, shaderFile.string());
    ...
    std::shared_ptr<ITexture> AssetManager::CreateTextureFromFile(UUID uuid, const std::string& name, const std::filesystem::path& textureFile, TextureType type, bool flipOnLoading)
    {
        if (s_AssetRegistry.find(s_AssetNameUUIDMap[name]) != s_AssetRegistry.end())
        {
            MOTION_CORE_WARN("Texture {0} already exists!", name);
        ...
        case RenderingAPI::OpenGL:
        {
            std::shared_ptr<ITexture> textureAsset = std::make_shared<GL_Texture>(uuid, name, textureFile, type, flipOnLoading);
            if (textureAsset)
            {
                s_AssetRegistry[textureAsset->GetMetaData().AssetUUID] = textureAsset;
                MOTION_CORE_INFO("Texture created: {0} from file {1}", name, textureFile.string());
    ...
    std::shared_ptr<ITexture> AssetManager::CreatePlainTexture(UUID uuid, const std::string& name, uint32_t width, uint32_t height)
    {
        if (s_AssetRegistry.find(s_AssetNameUUIDMap[name]) != s_AssetRegistry.end())
        {
            MOTION_CORE_WARN("Texture {0} already exists!", name);
        ...
        case RenderingAPI::OpenGL:
        {
            std::shared_ptr<ITexture> textureAsset = std::make_shared<GL_Texture>(uuid, name, width, height);
            if (textureAsset)
            {
                s_AssetRegistry[textureAsset->GetMetaData().AssetUUID] = textureAsset;
                MOTION_CORE_INFO("Texture created: {0}", name);
    ...
    std::shared_ptr<Model> AssetManager::LoadModel(UUID uuid, const std::string& name, std::filesystem::path& modelFile)
    {
        if (s_AssetRegistry.find(s_AssetNameUUIDMap[name]) != s_AssetRegistry.end())
        {
            MOTION_CORE_WARN("Model {0} already exists!", name);
        ...
        std::shared_ptr<Model> modelAsset = Importer::ImportModel(uuid, name, modelFile);
        if (modelAsset)
        {
            s_AssetRegistry[modelAsset->GetMetaData().AssetUUID] = modelAsset;
            ModelsManager::InsertModel(modelAsset->GetMetaData().AssetUUID, modelAsset);
