...
namespace Motion::Core
{
    ...
    static void InitLoggers()
    {
        ...
        s_CoreLogger = std::make_shared<spdlog::logger>("[MOTION::CORE]", std::begin(log_skin), std::end(log_skin));
        spdlog::register_logger(s_CoreLogger);
        s_CoreLogger->set_level(spdlog::level::trace);
        s_CoreLogger->flush_on(spdlog::level::trace);

        s_AppLogger = std::make_shared<spdlog::logger>("[MOTION::APP]", std::begin(log_skin), std::end(log_skin));
