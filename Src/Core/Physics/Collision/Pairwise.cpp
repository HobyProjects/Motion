...
namespace Motion
{
    ...
    static void BoxWorldData(const BoxShape& B, const glm::mat4& WB, glm::vec3& c, glm::vec3 axes[3], glm::vec3& eWorld)
    {
        glm::vec3 scales;
    ...
    static glm::vec3 FaceNormalLocal(const ConvexHullShape& H, const Tri& f) 
    {
    ...
    static std::uint32_t BestFaceAlong(const ConvexHullShape& H, const glm::mat4& W, const glm::vec3& dirWS) 
    {
    ...
    static void BuildFacePolygonWS(const ConvexHullShape& H, const glm::mat4& W, uint32_t faceIdx, std::vector<glm::vec3>& poly)
    {
    ...
    static SupportFn MakeSupportTriVsHull(const TempTriHull& tri, const glm::mat4& Wtri, const Collider& hull, const glm::mat4& Whull)
    {
        ...
        for (uint32_t idx : triIdx) 
        {
            ...
            switch (dynCol.ColliderShape->Type)
            {
                case ShapeType::Sphere:
                {
                    ...
                    const glm::vec3 nFace = TriNormalWS(a,b,c);
                    ...
                    if (M->BackfaceCull) 
                    {
                        if (glm::dot(nFace, diff) <= 0.0f) continue;
                    }
                ...
                case ShapeType::Convex:
                {
                    ...
                    const glm::vec3 nFace = TriNormalWS(a,b,c);
                    const glm::vec3 nEPA  = epa.Normal;
                    if (M->BackfaceCull) 
                    {
                        if (glm::dot(nFace, nEPA) <= 0.0f) continue;
                    } 
