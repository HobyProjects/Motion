...
{
    static bool LoadTextureFromFile(const std::filesystem::path& textureFile, bool flip) 
    {
    ...
    static bool GenerateTexture2D(uint32_t width, uint32_t height)
    {
    ...
    GL_TextureCubeMap::GL_TextureCubeMap(UUID uuid, const std::string & name, uint32_t width, uint32_t height)
    {
    }
