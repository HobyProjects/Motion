...
namespace Motion
{
    ...
    static inline void SHBasisL2(const glm::vec3& d, float outY[9])
    {
        ...
        outY[0] = c0; outY[1] = -c1 * y; outY[2] = c1 * z; outY[3] = -c1 * x;
        outY[4] = c2 * x * y; outY[5] = -c2 * y * z; outY[6] = c3 * (3.0f*z*z - 1.0f);
        outY[7] = -c2 * x * z; outY[8] = c4 * (x*x - y*y);
    }
    ...
    SH9 SH9::ProjectEquirectHDR(const std::filesystem::path& hdrFile)
    {
        ...
        if (!std::filesystem::exists(hdrFile)) return sh9;
        float* data = stbi_loadf(hdrFile.string().c_str(), &w, &h, &comp, 3);
        ...
        for(std::int32_t y = 0; y < h; ++y)
        {
            ...
            for(std::int32_t x = 0; x < w; ++x)
            {
                ...
                glm::vec3 dir = { glm::cos(phi) * glm::sin(theta), glm::cos(theta), glm::sin(phi) * glm::sin(theta) };
                float Y[9]; SHBasisL2(dir, Y);
                const std::int32_t idx = (y * w + x) * 3;
                const glm::vec3 L(data[idx + 0], data[idx + 1], data[idx + 2]);
                const float dOmega = dphi * dtheta * sinTheta;
                for(std::int32_t i = 0; i < 9; ++i) sh9.coeff[i] += L * (Y[i] * dOmega);
            }
        }
    ...
    {
        float Y[9]; SHBasisL2(glm::normalize(n), Y);
        glm::vec3 r(0.0f); for(std::int32_t i = 0; i < 9; ++i) r += sh.coeff[i] * Y[i];
        return r;
    }

    std::shared_ptr<IEnvironment> IEnvironment::Create(const EnvironmentSpecification & spec)
    {
