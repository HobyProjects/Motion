...
namespace Motion::Core
{
    ...
    {
    public:
        ModelThumbnail(const std::string& name, uint32_t width, uint32_t height, const std::shared_ptr<StaticMesh::SubMesh>& model);
    ...
    private:
        ...
        std::shared_ptr<StaticMesh> m_Model{ nullptr };
    ...
    {
    public:
        TextureThumbnail(const std::string& name, uint32_t width, uint32_t height, const std::shared_ptr<StaticMesh::SubMeshMaterial>& material);
    ...
    public:
        static std::shared_ptr<IThumbnail> CreateThumbnail(const std::string& name, const std::shared_ptr<StaticMesh>& model, uint32_t width = 128, uint32_t height = 128);
    ...
    public:
        ...
        ImTextureID GetOrGenerateForMesh(UUID id, const std::shared_ptr<StaticMesh::SubMesh>& mesh);
        ImTextureID GetOrGenerateForMaterial(UUID id, const std::shared_ptr<StaticMesh::SubMeshMaterial>& material);
